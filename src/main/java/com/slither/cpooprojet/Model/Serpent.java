...
public class Serpent {
    ...
    private double vitesse; // on peut changer la vitesse avec l'accélération
    private final Color couleur;
    private final Image skin;

    private Serpent() {
        this.segments = new ArrayList<SerpentPart>();
        this.segments.add(new SerpentPart(View.SCREENWIDTH / 2, View.SCREENHEIGHT / 2));
        this.vitesse = 2;
    ...
    private void setBody(double x, double y) {
        ...
        for (int i = 1; i < segments.size(); i++) {
            SerpentPart tmp = segments.get(i);
